    ...
    pub fn try_claim(
        ...
        for snip in config.snip120us {
            ...
            if let Some(amnt) = l2.may_load(deps.storage)? {
                ...
                let tc = TOTAL_CLAIMED.add_suffix(snip.addr.as_str().as_bytes());
                tc.update(deps.storage, |a| (Ok(a + amnt)))?;
...
mod tests {
    ...
    pub const HEADSTASH_DURATION: u64 = 420000000;
    ...
    fn default_eligilbe_eth_addrs() -> Vec<Headstash> {
        vec![
            Headstash {
    ...
    fn init_helper() -> (
        ...
        let init_msg = crate::msg::InstantiateMsg {
            ...
            start_date: None,
            end_date: Some(env.block.time.plus_seconds(HEADSTASH_DURATION).seconds()),
            // snip120u_code_id: 2,
    ...
    fn test_init_sanity() {
        ...
        assert_eq!(
            constants.end_date,
            Some(env.block.time.seconds() + HEADSTASH_DURATION)
        );
    ...
    fn test_headstash_workflow() {
        ...
        let handle_msg = ExecuteMsg::AddEligibleHeadStash {
            headstash: default_eligilbe_eth_addrs(),
        };
    ...
    fn test_clawback_sanity() {
        ...
        // try to clawback with non owner
        ...
        .expect_err(&ContractError::ClawbackUnavailable {}.to_string());

        // clawback tokens, expecting to mint 320 scrt TERP & 710 scrt THIOL
        env.block.time = env.block.time.plus_seconds(HEADSTASH_DURATION + 1);
        let res = execute(
            deps.as_mut(),
            env.clone(),
            info.clone(),
        ...
        let _ = res.events[0].attributes.iter().map(|a| {
            if a.key == "snip1Addr" {
                assert_eq!(a.value, "320")
            } else if a.key == "snip2Addr" {
                assert_eq!(a.value, "710")
            }
        });
    }
}
