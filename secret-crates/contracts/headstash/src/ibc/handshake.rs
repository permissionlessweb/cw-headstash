...
mod ibc_channel_open {
    use crate::{
        state::{self, channel::ChannelState},
        types::callbacks::IcaControllerCallbackMsg,
    };

    use super::{
        keys, ContractError, DepsMut, Ibc3ChannelOpenResponse, IbcBasicResponse, IbcChannel,
    ...
    pub fn on_acknowledgement(
        ...
        if let Some(callback_address) = state.callback_contract {
            ...
            .into_cosmos_msg(callback_address.address, callback_address.code_hash)?;
...
mod ibc_channel_close {
    use crate::state::{self, channel::ChannelStatus};

    use super::{ContractError, DepsMut, IbcBasicResponse, IbcChannel};
