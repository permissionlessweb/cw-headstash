...
pub mod from_protobuf {
    ...
    use cosmwasm_std::{
        AllBalanceResponse, BalanceResponse, Binary, Coin, ContractInfoResponse, StdResult, Uint128,
    };
        ...
        match path {
            constants::VALIDATOR => {
                ...
                Ok(IcaQueryResponse::Staking(StakingQueryResponse::Validator(
                    ValidatorResponse {
                        validator: resp.validator.map(convert_to_validator).transpose()?,
                    },
                )))
            }
            constants::ALL_VALIDATORS => {
                ...
                Ok(IcaQueryResponse::Staking(
                    StakingQueryResponse::AllValidators(AllValidatorsResponse {
                        validators: resp
                            .validators
                            .into_iter()
            ...
            constants::STAKING_PARAMS => {
                ...
                Ok(IcaQueryResponse::Staking(
                    StakingQueryResponse::BondedDenom(BondedDenomResponse {
                        denom: resp
                            .params
